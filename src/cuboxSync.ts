...
function generatePageTitle(article: CuboxArticle): string {
  ...
  const date = formatDate(article.create_time, "yyyy-MM-dd");
...
function generateArticleBlocks(article: CuboxArticle): LogseqBlock[] {
  ...
  const metadataLines = [
    ...
    `**Domain:** ${article.domain}`,
    `**Type:** ${article.type}`,
    `**Created:** ${formatDate(article.create_time)}`,
    `**Updated:** ${formatDate(article.update_time)}`
  ];

  if (article.description) {
    metadataLines.push(`**Description:** ${article.description}`);
  }

  if (article.tags && article.tags.length > 0) {
    metadataLines.push(`**Tags:** ${article.tags.map(tag => `#${tag}`).join(" ")}`);
  }

  blocks.push({ content: metadataLines.join("\n") });
